...
class VueLogger implements ILogger {
  private errorMessage: string =
    "Provided options for vuejs-logger are not valid.";
  private logLevels: string[] = Object.keys(LogLevels).map((l) =>
    l.toLowerCase(),
  );
  ...
  private changeLogLevel(aLogLevel: string) {
    this.logLevels.forEach((logLevel) => {
      if (
        this.logLevels.indexOf(logLevel) >= this.logLevels.indexOf(aLogLevel)
      ) {
        this.vueInstance.$log[logLevel] = this.createLoggerObject(
          this.options,
          logLevel,
        );
      } else {
  ...
  public isValidOptions(options: ILoggerOptions, logLevels: string[]): boolean {
    ...
    if (
      ...
      typeof options.stringifyArguments !== "boolean"
    ) {
      return false;
    }
    ...
    if (
      ...
      typeof options.showConsoleColors !== "boolean"
    ) {
      return false;
    }
    if (
    ...
    return !(
      options.showMethodName && typeof options.showMethodName !== "boolean"
    );
  }
  ...
  private printLogMessage(
    ...
    if (
      ...
      console[logLevel === "fatal" ? "error" : logLevel](
        logMessage,
        ...formattedArguments,
      );
    } else {
